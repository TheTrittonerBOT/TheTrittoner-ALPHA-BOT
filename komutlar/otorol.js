...
exports.run = async (bot, message, args) =>
{
  ...
  if (!mentionedChannel && args[0] !== "sıfırla") return message.channel.send("Ayarlamam İçin Bir Rol Etiketlemelisin. \nRolü Etiketleyemiyorsan **Rolün Etiketleme Seçeneğini Aktif Etmeyi Unutma** \nÖrnek Kullanım : t!otorol @rol #kanal \n**Oto Yazı Kapatmak İstiyorsan t!kapat otoyazı**");
  if (message.guild.member(message.author.id).hasPermission(0x8))
    
    {
      var mentionedRole = message.mentions.roles.first();
      if (!mentionedRole) return message.channel.send("**Doğru Kullanım = t!otorol @<roladı> #<metinkanalı>**".then(msg => msg.delete(5000)));
